...
use windows::Win32::Devices::HumanInterfaceDevice::{
    HidD_FreePreparsedData, HidD_GetAttributes, HidD_GetFeature, HidD_SetFeature, HidD_GetInputReport, HidD_GetPreparsedData, HidD_GetProductString, HidD_GetManufacturerString, HidD_GetSerialNumberString, HidP_GetCaps, HIDD_ATTRIBUTES, HIDP_CAPS, HIDP_STATUS_SUCCESS, PHIDP_PREPARSED_DATA
};
...
impl Device {
    ...
    pub fn name(&self) -> HidResult<String> {
        let product = self.read_string(HidD_GetProductString)
            .ok_or_else(|| windows::core::Error::from_win32())?;
        let manufacturer = self.read_string(HidD_GetManufacturerString)
            .ok_or_else(|| windows::core::Error::from_win32())?;
        // return same format as hidraw backend
    ...
    pub fn send_feature_report<'a>(&self, buf: &'a [u8]) -> HidResult<()> {
        check_error(unsafe { HidD_SetFeature(self.0, buf.as_ptr() as _, buf.len() as u32) })?;
        Ok(())
    }
}
